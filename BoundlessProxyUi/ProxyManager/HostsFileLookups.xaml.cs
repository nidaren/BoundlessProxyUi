        ...
        private void UserControl_Loaded(object sender, RoutedEventArgs e)
        {
            new Thread(() =>
            {
                try
                {
                    ...
                    try
                    {
                        serverListJson = JArray.Parse(Network.NidHttpClient.GetAsync($"https://{Constants.DiscoveryServer}:8902/list-gameservers").Result.Content.ReadAsStringAsync().Result);
                    }
                    catch (Exception ex)
                    {
